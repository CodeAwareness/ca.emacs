...
(defun test-peer-selection-handling ()
  ...
  (let ((test-peer-data '((_id . "67bc3432cf9b5efe459008dd")
                          ...
                          (lang . "en"))))
    ;; Test peer selection
    (codeawareness--handle-peer-select test-peer-data)
    (assert (equal codeawareness--selected-peer test-peer-data)
            "Selected peer should be set correctly")
    
    ;; Test peer unselection
    (codeawareness--handle-peer-unselect)
    (assert (null codeawareness--selected-peer)
...
(defun test-diff-buffer-creation ()
  ...
  (let ((test-peer-file "/tmp/test-peer-file.txt")
    ...
    (codeawareness--open-diff-view test-peer-file test-user-file test-title)
...
(defun test-ipc-message-parsing ()
  ...
  (let ((test-message "{\"flow\":\"req\",\"domain\":\"code\",\"action\":\"peer:select\",\"data\":{\"_id\":\"67bc3432cf9b5efe459008dd\",\"name\":\"Alice\",\"company\":\"Code Awareness\",\"phone\":\"123-123-1234\",\"email\":\"alice@codeawareness.com\",\"createdAt\":\"2025-02-24T08:56:18.540Z\",\"active\":true,\"updatedAt\":\"2025-08-29T00:38:56.447Z\",\"lang\":\"en\"},\"err\":\"\",\"caw\":\"0\"}"))
    ;; Mock the peer selection handler to track calls
    (let ((peer-select-called nil))
      (flet ((codeawareness--handle-peer-select (data)
               (setq peer-select-called t)
               (assert (equal (alist-get 'name data) "Alice")
                       "Peer name should be parsed correctly")))
        (codeawareness--handle-ipc-message test-message)
...
(defun test-event-handler-registration ()
  ...
  (assert (gethash "peer:select" codeawareness--events-table)
          "peer:select handler should be registered")
  (assert (gethash "branch:select" codeawareness--events-table)
          "branch:select handler should be registered")
  (assert (gethash "auth:logout" codeawareness--events-table)
